...
const RadioGroup = ({ name, size = "reg", selected, radioButtons, layout = "vertical" }) => {
  return (
    <>
      {radioButtons.map(({ label, value, disabled }, index) => (
        <div key={index} className={layout === "vertical" ? "block" : "inline-block"}>
          ...
          >
            <input type="radio" name={name} checked={selected === value} id={value} disabled={disabled} value={value} />
